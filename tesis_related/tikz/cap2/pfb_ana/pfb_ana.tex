...
%	};
	...
	{
		\node[coordinate] at (2,\i)            (m81) {};  
		\node[dspmixer] at (3,\i)              (m82) {};          
		\node[dspsquare] at (4,\i)            (m83) {$h_\i(n)$};    
		\node[dspsquare] at (5,\i)            (m84) {\downsamplertext{M}}; \\}
%		\node[dsp/label=north]       (m15) {$X_0(m)$};          &
%				\node[dsp/label=north]       (m16) {$\hat{X}_0(m)$};          &
%    \node[dspsquare]             (m17) {\upsamplertext{M}}; &
%    \node[dspsquare]             (m18) {$f(n)$};    &
...
%		\draw[dspconn] (c\i) -- (c\j);
	\node[coordinate] at (0,2) (P) {P};
	\draw[solid,thick,->] (P) arc (-90:-180:-1);
\end{tikzpicture}
