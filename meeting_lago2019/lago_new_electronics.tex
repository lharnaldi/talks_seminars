...
%el abstract que subí para este documento es 
%The new data acquisition system (DAQ), based on the RedPitaya board is
%presented. It is shown the preliminary results obtained in the development of
%the new DAQ system. It is designed to be flexible enough for adapting to
%different  particle detectors, such as particle detectors based on scintillators
%or   resistive plate cameras (RPC). According to the requirements of  the water
%Cherenkov detectors (WCD) used in LAGO, the new system must be  capable of
%recording fast pulses (~ ns), in addition to monitoring the  atmospheric
%conditions in which the data were taken. Some of the  figures of merit are
%shown, indicating the performance of the new DAQ working with WCD. The DAQ
%system is based on a comercial, plus a custom made interface board. The
%implementation includes scalers, sub-scalers, an automatic baseline correction
%algorithm, pressure and temperature sensing, geolocalization and an external
%trigger. The first results proves the validity, reliability and high performance
%of the system.
\begin{document}
...
\begin{exampleblock}{}
\begin{itemize}
\item En experimentos: recuperación de señales pequeñas en ambientes ruidosos
%Ejemplo, se puede recuperar una señal de 1uV en otra de 1mV
\item Mejora en la relación Señal/Ruido (SNR)
\end{itemize}
\end{exampleblock}
  \begin{columns}
    \begin{column}{0.45\textwidth}
      \begin{block}{}
        \begin{itemize}[<+->]
          \item Utiliza una técnica conocida como detección sensible a la fase\\
          \item Instrumento sofisticado
          \item Puede tener valor monetario elevado, dependiendo de las
prestaciones
\item Una de las herramientas más empleada en los laboratorios de investigación
en física
        \end{itemize}
      \end{block}
    \end{column} \ \
    \begin{column}{0.50\textwidth}
      \only<1>{\begin{alertblock}{}Las mediciones en un lock-in necesitan una
{\color{red}frecuencia de referencia}. Típicamente un experimento se excita a una frecuencia
dada (utilizando un oscilador o generador de funciones) y el lock-in
{\color{blue}detecta la respuesta del experimento a la frecuencia de
referencia}.\end{alertblock}}
      \only<2>{\includegraphics[width=0.8\textwidth]{freq_ref}}
      \only<3-4>{\includegraphics[width=\textwidth]{SR530}}
    \end{column}
  \end{columns}
\end{frame} 
...
\begin{column}{0.5\textwidth}
\begin{center}
\only<1>{\includegraphics[width=0.7\textwidth]{meas_capacitance}\\
         \includegraphics[width=0.7\textwidth]{meas_resistance}}
\only<2>{\includegraphics[width=0.95\textwidth]{nmr}}
\only<3-4>{\includegraphics[width=0.95\textwidth]{afm}}
\end{center}
\end{column}
\end{columns}
\end{frame}  
...
%------------------------------------------------------------------------------
\subsection{El estándar AXI}
%------------------------------------------------------------------------------

\begin{frame}
  \frametitle{{\color[rgb]{0.00,0.21,0.47}A}dvanced e{\color[rgb]{0.00,0.21,0.47}X}tensible {\color[rgb]{0.00,0.21,0.47}I}nterface}
  \begin{exampleblock}{}
  Forma parte del estándar abierto ARM AMBA\textsuperscript{\textregistered} 3.0
  \end{exampleblock}
  \begin{columns}
    \begin{column}{0.35\textwidth}
      \begin{block}{}
Hoy en día hay tres sabores de AXI4, cada uno de los
cuales representa un protocolo de bus diferente. La
elección del protocolo de bus AXI para una conexión
determinada depende de las propiedades deseadas de
esa conexión.
      \end{block}
    \end{column} \ \
    \begin{column}{0.55\textwidth}
      \only<1>{\begin{alertblock}{\textbf{AXI4}} Para interconexiones
mapeadas en memoria; proporciona el mayor rendimiento: se provee una dirección,
seguida  por una transferencia de ráfaga de datos de hasta 256 palabras
\end{alertblock}}
      \only<2>{\begin{alertblock}{\textbf{AXI4-Lite}} Interconexión simplificada
que soporta sólo una transferencia de datos por conexión (sin ráfagas).
AXI4-Lite también es mapeado en memoria: en este caso se transfieren una dirección y una
sola palabra de datos \end{alertblock}}
      \only<3-4>{\begin{alertblock}{\textbf{AXI4-Stream}}
Para transmisión de datos de alta velocidad, soporta transferencias de ráfaga
sin restricciones de tamaño. No hay ningún mecanismo de dirección. Este tipo de
interconexión es el más adecuado para el flujo de datos directo entre la fuente
y el destino (sin mapeo en memoria)\end{alertblock}}
    \end{column}
  \end{columns}
\end{frame}
...
%\begin{block}{}
\begin{center}
\includegraphics[width=\textwidth]{sa_sin_cos}
\end{center}
%\end{block}
\end{frame} 
...
%\begin{block}{}
\begin{center}
\includegraphics[width=\textwidth]{gen_y_osc2}
\end{center}
%\end{block}
\end{frame} 
...
%\begin{frame}
%\frametitle{¿Qué es un amplificador lock-in?}
%{
